...
`server/blog/mod.tsx` reads every `.md` file beside it, sorts them by date, and
...
`server/blog/mod.tsx` is the whole of it: the frontmatter parser, the file reads,
...
`router.tsx` maps the blog's routes to that module and never learns that Markdown
